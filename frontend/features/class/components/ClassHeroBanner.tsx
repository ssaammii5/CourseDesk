...
export function ClassHeroBanner({
    title,
    details,
    course,
    nextSession,
    isTeacher,
}: ClassHeroBannerProps) {
    ...
    const primaryTeacher = useMemo(() => {
        if (course?.teacherNames && course.teacherNames.length > 0) {
            return course.teacherNames[0];
        }
        const teacherPerson = details.people.find((p) => p.role === "Teacher");
        return teacherPerson?.name || "Instructor";
    }, [course, details.people]);

    // Student count
    const studentCount =
        course?.studentCount ??
        details.people.filter((p) => p.role === "Student").length;
            ...
            >
                ...
                <div className="relative z-10 flex flex-col justify-between gap-4 p-5 sm:p-6">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3 pt-1 border-t border-white/5">
                        {/* Instructor & Student Meta */}
                        <div className="flex flex-wrap items-center gap-4 text-xs sm:text-sm text-slate-300">
                            <div className="flex items-center gap-1.5">
                                <GraduationCap className="h-4 w-4 text-indigo-400" />
                                <span className="font-medium text-slate-200">{primaryTeacher}</span>
                            </div>
                        ...
                        <div className="mt-5 divide-y divide-gray-100 text-sm">
                            ...
                            <div className="flex justify-between py-2.5">
                                ...
                                <span className="font-medium text-gray-900">
                                    {course?.teacherNames?.join(", ") || primaryTeacher}
                                </span>
                            </div>
